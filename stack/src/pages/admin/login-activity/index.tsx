...
<div className="flex flex-col gap-3 rounded-xl border bg-white p-4 shadow-sm sm:flex-row">
  <input
    ...
    placeholder="Search by name, username, email, or IP address..."
    className="flex-1 rounded-lg border border-gray-300 px-4 py-2.5 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
  />
...
>
            ...
            {filteredActivities.map((activity) => (
              <div key={activity._id} className="p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  >
                    {activity.isRevoked ? "Revoked" : "Active"}
                  </span>
                </div>
  ...
  {activity.isTrustedDevice ? "Yes" : "No"}
</p>
...
<p>
  ...
  {new Date(activity.expiresAt).toLocaleString()}
                ...
                <div className="mt-3 text-xs text-gray-500">
                  Login: {new Date(activity.loginAt).toLocaleString()}
                  {" · "}
                  Last active:{" "}
